...
const SideLongbannerController = {
  uploadBanner: async (req, res) => {
    try {
      ...
      const dropboxPath = `/tabbanners/${req.file.filename}`;
      ...
      const updateQuery = `UPDATE Tab_banners SET file_url = ? WHERE id = 4`;
  ...
  getBanner: async (req, res) => {
    try {
      const [rows] = await pool.query(
        "SELECT file_url FROM Tab_banners WHERE id = 4"
      );
...
module.exports = SideLongbannerController;
